...
class RPA4Browser():
    ...
    def __init__(self, view:tk.Frame) -> None:
        ...
        try:
            ...
            self.view.btn_voice.config(state='disable')
        else:
            self.view.btn_voice.bind('<1>', self.voice_ctl_th)
    ...
    def key_type(self, event=None, key:str=None):
        ...
        """
        if self.view:
            self.view.set_btn_bg_color(key, True)   # ボタンの背景色を操作中の色に設定

        # キー入力用辞書(キーストローク名:入力するキー)
        key_stroke = {"bottom":"{END}", "top":"{HOME}", "down":"{PGDN}", "up":"{PGUP}"
                    , "next_page":"次のページ", "search":"検索", "jump":"{TAB}"
                    , "back":"%{LEFT}", "see":"~", "close":"^w"}
        ...
        if self.view:
            self.view.set_btn_bg_color(key, False)   # ボタンの背景色を未操作の色に設定
    ...
    def voice_ctl_th(self, event=None):
        """
        voice_ctl用スレッド
        """
        th = threading.Thread(target = self.voice_ctl, args=(event,), daemon=True)
        th.start()

    def voice_ctl(self, event=None):
        """
        音声を認識してコマンドを出す
        「終わり」と認識するまで続ける
        """
        # 画面で使用しているコマンド用辞書をキーと値を入れ替えて辞書にする
        cmd_dic = {v:k for k,v in self.view.commands.items()}
        cmd_dic["終わり"] = "OWARI"

        self.view.btn_voice.config(bg="lightgreen")
        self.view.btn_voice.update_idletasks()

        while True:
            self.vr.listen_voice(1)                   # 音声認識を起動
            voice_cmd = self.vr.futures[-1].result()  # 認識した音声を文字で取得
            self.view.var_voice.set(voice_cmd)
            logger.info(f"認識文字：{voice_cmd}")
            if voice_cmd in cmd_dic:
                if voice_cmd == "終わり": break
                self.key_type(key=cmd_dic[voice_cmd])
            if voice_cmd.endswith("を検索"):
                self.view.var_word.set(voice_cmd.rstrip("を検索"))
                self.key_type(key=cmd_dic["検索"])

        self.view.btn_voice.config(bg="SystemButtonFace")
        self.view.btn_voice.update_idletasks()
...
class MyFrame(tk.Frame):
    ...
    def __init__(self, master) -> None:
        ...
        # 制御用ボタンの作成
        self.commands = {"down":"下に", "next_page":"次のページ", "up":"上に"
                        , "top":"一番上に", "bottom":"一番下に", "jump":"次のリンク"
                        , "back":"戻る", "see":"表示", "close":"閉じる", "search":"検索"}
        self.buttons = {}
        for key, value in self.commands.items():
            self.buttons[key] = tk.Button(self, text=value)
            self.buttons[key].bind("<1>", lambda event, key=key: self.rpa.key_type(event, key))
            self.buttons[key].bind("<space>", lambda event, key=key: self.rpa.key_type(event, key), add=True)
            self.buttons[key].bind("<Return>", lambda event, key=key: self.rpa.key_type(event, key), add=True)
            self.buttons[key].pack(fill=tk.X)
        
        # 検索ボタンとエントリーの作成
        self.buttons["search"].pack(pady=(5,0))
        self.var_word = tk.StringVar(value="")
        ent_word = tk.Entry(self, textvariable=self.var_word)
        ent_word.pack(fill=tk.X)

        # 音声認識ボタンと認識内容表示ラベルの作成
        self.btn_voice = tk.Button(self, text='音声認識')
        self.btn_voice.pack(fill=tk.X)
        self.var_voice = tk.StringVar(value="")
        lbl_voice = tk.Label(self, textvariable=self.var_voice)
        lbl_voice.pack(fill=tk.X)

    def set_btn_bg_color(self, key:str, in_act:bool):
        """
        ボタンの背景色を設定する
        """
        _colors = {False:"SystemButtonFace", True:"lightgreen"}
        self.buttons[key].config(bg=_colors[in_act])
        self.buttons[key].update_idletasks()
